...
/**
 * Verbessert die Anmerkungen für "Nur im Scan"-Felder mit Claude Haiku.
 * Optional: wird nur aufgerufen wenn ANTHROPIC_API_KEY gesetzt ist.
 * Modell: claude-haiku-4-5-20251001 (günstigstes Modell, ~$0.001 pro Lauf)
 */
async function enhanceWithClaude(comparison, scanResult) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) {
    console.log('(Claude Enhancement übersprungen — kein ANTHROPIC_API_KEY gesetzt)');
    return comparison;
  }

  const nurImScanId    = comparison.idVergleich.filter(r => r.status === 'nurScan');
  const nurImScanLabel = comparison.labelVergleich.filter(r => r.status === 'nurScan');

  if (nurImScanId.length === 0) {
    console.log('Keine "Nur im Scan"-Felder — Claude Enhancement nicht nötig.');
    return comparison;
  }

  // Kontext der neuen Felder aus dem Scan holen
  const scanMap = new Map(scanResult.fields.map(f => [f.id, f]));
  const neueFelder = nurImScanId.map(r => {
    const sf = scanMap.get(r.idScan);
    return {
      id:            r.idScan,
      label:         sf?.label || '',
      seite:         r.anmerkung.split('|')[0]?.trim() || '',
      position_y:    sf?.position_y || 0,
      foundInContext: sf?.foundInContext || '',
      fieldType:     sf?.fieldType || '',
    };
  });

  // Alle Felder im Scan für Nachbar-Kontext (nur IDs + Labels)
  const bekannteFelder = scanResult.fields
    .filter(f => !nurImScanId.some(r => r.idScan === f.id))
    .map(f => ({ id: f.id, label: f.label, position_y: f.position_y }));

  const Anthropic = require('@anthropic-ai/sdk');
  const client = new Anthropic({ apiKey });

  const prompt = `Du bist Experte für das Formular 1400 der Bundesfinanzverwaltung (Stromsteueranmeldung).

Im automatischen Scan wurden folgende NEUE Felder gefunden, die noch NICHT im Erfassungsbogen (Excel) eingetragen sind:

${JSON.stringify(neueFelder, null, 2)}

Bekannte Nachbar-Felder im Formular (zur Positionsbestimmung):
${JSON.stringify(bekannteFelder.slice(0, 80), null, 2)}

Analysiere jedes neue Feld und antworte mit einem JSON-Array:
[
  {
    "id": "FeldID",
    "einschätzung": "Kurze Einschätzung was dieses Feld bedeutet (1 Satz)",
    "position": "Wo im Formular: Seite, Abschnitt, Nachbar-Felder",
    "empfehlung": "Soll in Erfassungsbogen aufgenommen werden? Warum?",
...
Antworte NUR mit dem JSON-Array, kein weiterer Text.`;
  ...
  console.log(`Claude Haiku analysiert ${neueFelder.length} neue Felder...`);
  ...
  try {
    ...
    console.warn('Claude Enhancement fehlgeschlagen:', err.message);
    return comparison;
  }

  // Anmerkungen in idVergleich + labelVergleich updaten
  const enhancedMap = new Map(parsed.map(e => [e.id, e]));

  for (const row of comparison.idVergleich) {
    if (row.status !== 'nurScan') continue;
    const e = enhancedMap.get(row.idScan);
    if (e) {
      row.anmerkung = `[${e.priorität?.toUpperCase()}] ${e.einschätzung} | ${e.position} | Empfehlung: ${e.empfehlung}`;
    }
  }

  for (const row of comparison.labelVergleich) {
    if (row.status !== 'nurScan') continue;
    const e = enhancedMap.get(row.feldId);
    if (e) {
      row.anmerkung = `[${e.priorität?.toUpperCase()}] ${e.empfehlung}`;
    }
  }

  console.log(`✓ Claude Enhancement abgeschlossen (${parsed.length} Felder analysiert).`);
  return comparison;
}

module.exports = { parseExcel, runComparison, generateExcel, enhanceWithClaude };
